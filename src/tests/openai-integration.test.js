...
describe('OpenAI API Integration', () => {
  beforeAll(() => {
    if (!OPENAI_API_KEY || OPENAI_API_KEY === 'YOUR_API_KEY_HERE') {
      throw new Error('OpenAI API key not configured. Please set it in src/lib/config.js');
    }
  });
...
Please provide a comprehensive tarot reading.`
    ...
    const temporalReferences = [
      reading.includes('past') || reading.includes('was') || reading.includes('before'),
      reading.includes('present') || reading.includes('now') || reading.includes('current'),
      reading.includes('future') || reading.includes('will') || reading.includes('ahead'),
    ];
